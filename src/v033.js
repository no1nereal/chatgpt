...
async function runLiveScout(env) {
  ...
  const messages = [
    {
      ...
      content: [
        ...
        "Today is 2026-08-13.",
        "Research the live web for self-serve digital products or micro-SaaS opportunities that could plausibly become mostly autonomous.",
  ...
  for (let step = 0; step < 6; step++) {
    ...
    for (const toolCall of toolCalls) {
      ...
      // Count before execution. One POST only; no automatic retry on a search execution.
      searchCalls += 1;
...
<body>
<main>
  <div class="eyebrow">private control surface · v0.3.3</div>
  <h1>sefi foundry</h1>
  <p class="sub">the creature can think and scout the live web. hard tool budgets are enforced outside the model. it still cannot spend money, contact people, deploy products, or touch payment accounts.</p>
  ...
  <section class="panel">
    <div class="safe">
      ...
      <span class="amber">scout: max 4 executed searches, excess requests denied</span>
    </div>
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/chairman/ping") {
      ...
      return json({ chairman: "recognized", foundry: "ready", version: "0.3.3" });
    }
